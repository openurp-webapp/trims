...
class TeachingAction extends RestfulAction[Person] {
  ...
  def lessonIndex(@param("id") id: Long): String = {
  ...
  def lesson(@param("id") id: Long, @param(value="year",required=false) curYear: String): String = {
  ...
  private def getSchoolYears(id: Long): Seq[Any] = {
  ...
  def grade(@param("id") id: Long): String = {
    lessonIndex(id)
    val curYear = attribute("curYear").asInstanceOf[String]
  ...
  //期末成绩examGrade.end
  private def putExamGrade(id: Long, curYear: String) {
  ...
  //总评成绩gaGrade.endGa
  private def putGaGrade(id: Long, curYear: String) {
    val sql = s"""select id,score,count(*) from
			(select l.id, case 
			when cg.score>=60 then 1
			else 0 end  score
  ...
  def qualityYear(@param("id") id: Long): String = {
    lessonIndex(id)
    val curYear = attribute("curYear")
