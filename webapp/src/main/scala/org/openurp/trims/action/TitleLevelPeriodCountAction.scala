...
class TitleLevelPeriodCountAction extends AbsEamsAction {
  ...
  def search(): String = {
    ...
    val sql = """select b.level_id,cast(avg(num) as int) num from
		(select a.user_id,a.level_id,cast(avg(num) as int) num from 
		(select lt.user_id,pt.level_id, sum(c.period) num
    ...
    (if(endYear.isDefined)s" and l.semester_id <= '${endYear.get}'"else"")+
		""" group by lt.user_id,pt.level_id,s.id)a
		group by a.level_id,a.user_id)b
		group by b.level_id
		order by num desc"""
