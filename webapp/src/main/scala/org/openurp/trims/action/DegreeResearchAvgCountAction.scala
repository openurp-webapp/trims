...
import org.openurp.base.code.DegreeLevel
...
class DegreeResearchAvgCountAction extends AbsEamsAction {
  ...
  def thesis(): String = {
    ...
    val sql = """select level_id,cast(sum(num)/count(*) as int) num from
      (select degree.level_id,t.researcher_id,count(*) num
      ...
      """ group by degree.level_id,t.researcher_id) a
      group by level_id
      order by num desc"""
  ...
  def literature(): String = {
    ...
    val endYear = get("endYear")
    val sql = """select level_id,cast(sum(num)/count(*) as int) num from
        (select degree.level_id,l.researcher_id,count(*) num
        ...
        """ group by degree.level_id,l.researcher_id)a
        group by level_id
        order by num desc"""
    val query = SqlBuilder.sql(sql)
