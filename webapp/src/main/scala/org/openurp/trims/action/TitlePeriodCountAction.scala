...
class TitlePeriodCountAction  extends AbsEamsAction {
  ...
  def search(): String = {
    ...
    (select a.user_id,a.title_id,cast(avg(num) as int) num from 
    (select lt.user_id,post.title_id, sum(c.period) num
    ...
    join hr_base.staffs staff on staff.person_id=lt.user_id
    ...
    """ group by lt.user_id,post.title_id,s.id)a
    group by a.title_id,a.user_id)b
    group by b.title_id
    order by num desc"""
  ...
  def department(): String = {
    ...
    (select a.department_id,a.user_id,cast(avg(num) as int) num from 
    (select post.department_id,lt.user_id, sum(c.period) num
    from edu_teach.lessons l 
    join edu_teach.lessons_teachers lt on lt.lesson_id=l.id
    join base.semesters s on l.semester_id = s.id 
    join edu_base.courses c on c.id = l.course_id
    ...
    (if(endYear.isDefined)s" and l.semester_id <= '${endYear.get}'"else"")+
      """
    group by lt.user_id,post.department_id,s.id)a
    group by a.department_id,a.user_id)b
  ...
  def title(): String = {
    ...
    (select p.name p_name,lt.user_id,s.school_year,s.name, sum(c.period) num
    from edu_teach.lessons l 
    join edu_teach.lessons_teachers lt on lt.lesson_id=l.id
    join base.semesters s on l.semester_id = s.id 
    join edu_base.courses c on c.id = l.course_id
    join hr_base.staffs staff on staff.person_id=lt.user_id
    join hr_base.staff_post_infoes post on staff.post_head_id=post.id
    join base.users p on staff.person_id = p.id
    where staff.state_id = 1""" +
    (if (tid == 0) "and post.title_id is null" else " and post.title_id=" + tid) +
    (if(beginYear.isDefined)s" and l.semester_id >= ${beginYear.get}"else"")+
    (if(endYear.isDefined)s" and l.semester_id <= '${endYear.get}'"else"")+
    s""" and post.department_id=${did}
    group by lt.user_id,p.name,s.school_year,s.name)a
    group by a.p_name
    order by num desc"""
    val query = SqlBuilder.sql(sql)
    val datas = entityDao.search(query)
