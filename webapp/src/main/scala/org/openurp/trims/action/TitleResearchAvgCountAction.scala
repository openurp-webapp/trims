...
class TitleResearchAvgCountAction extends AbsEamsAction {
  ...
  def thesis(): String = {
    ...
    val sql = """select title_id,cast(sum(num)/count(*) as int) num from
      (select te.title_id,t.researcher_id,count(*) num
      ...
      """ group by te.title_id,t.researcher_id) a
      group by title_id
      order by num desc"""
  ...
  def literature(): String = {
    ...
    val endYear = get("endYear")
    val sql = """select title_id,cast(sum(num)/count(*) as int) num from
        (select te.title_id,l.researcher_id,count(*) num
      ...
      """ group by te.title_id,l.researcher_id)a
      group by title_id
      order by num desc"""
    val query = SqlBuilder.sql(sql)
